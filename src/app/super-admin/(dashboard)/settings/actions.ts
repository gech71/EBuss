...
export async function createUserAction(formData: FormData) {
  ...
  if (!superAdmin || superAdmin.role !== "SUPER_ADMIN") {
    await logAction({
      actionType: "CREATE_ADMIN_USER_UNAUTHORIZED",
      description: "Unauthorized attempt to create admin user.",
    });
  ...
  try {
    ...
    await logAction({
      ...
      description: `Created new admin user '${name}' (${newUser.id}) for owner ${ownerId}.`,
    });
    ...
    try {
      ...
      console.error(
        "Failed to send setup email for new admin user, but user was created successfully.",
        emailError,
      );
    }
    ...
    await logAction({
      userId: superAdmin.id,
      actionType: "CREATE_ADMIN_USER_FAIL",
      description: `Error creating user. Error: ${error instanceof Error ? error.message : "Unknown"}`,
    });
    return {
      success: false,
