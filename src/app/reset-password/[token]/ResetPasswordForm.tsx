  ...
  const handleSubmit = (formData: FormData) => {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        // On success, the action logs the user in, redirect to appropriate page
        router.push('/admin');
      } else {
        toast({
          title: "Reset Failed",
  ...
  return (
    <form ref={formRef} action={handleSubmit} className="space-y-6">
      ...
      <CardFooter className="p-0 pt-6">
        <Button type="submit" disabled={isButtonDisabled} className="w-full" id="reset-password-submit">
            {isPending ? "Resetting Password..." : "Reset Password and Login"}
        </Button>
      </CardFooter>
    </form>
  );
