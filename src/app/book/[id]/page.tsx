...
export default async function BookPage({ params }: BookPageProps) {
  const { id: routeId } = (await params) as { id: string };
  ...
  const routeData = await prisma.route.findUnique({
    ...
    include: {
      ...
      tickets: {
        // Fetch tickets to determine seat availability
        where: {
  ...
  const route = {
    ...
    roundTripDiscountValue: routeData.roundTripDiscountValue
      ? Number(routeData.roundTripDiscountValue)
      : null,
    discount: routeData.discount
      ? {
          ...routeData.discount,
          percentage: routeData.discount.percentage
            ? Number(routeData.discount.percentage)
            : null,
          tiers: routeData.discount.tiers.map((tier) => ({
            ...tier,
  ...
  const alternativeRoutes = [routeData, ...alternativeRoutesData].map(
    (altRoute) => ({
      ...altRoute,
