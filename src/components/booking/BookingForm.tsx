...
type EnrichedDiscount = Discount & {
  tiers: DiscountTier[];
  percentage: number | null;
};

type ClientSeat = Seat & { status?: "AVAILABLE" | "OCCUPIED" | "SELECTED" };

type RouteWithDetails = Route & {
  origin: Location;
  destination: Location;
  bus: Bus & { layout: SeatLayout & { seats: ClientSeat[] } };
...
type ReturnRoute = Route & {
  ...
  bus: Bus & {
    owner: { name: string };
    layout: SeatLayout & { seats: ClientSeat[] };
  };
};
...
export function BookingForm({
  ...
  const [selectedRoute, setSelectedRoute] =
    useState<RouteWithDetails>(initialRoute);
  ...
  const [selectedReturnRoute, setSelectedReturnRoute] =
    useState<ReturnRoute | null>(null);
  ...
  const areSeatsAvailable = useMemo(() => {
    ...
    return selectedRoute.bus.layout.seats.some(
      (seat) =>
        seat.type === "SEAT" && !occupiedSeatNumbers.has(seat.seatNumber),
    );
  }, [selectedRoute]);
  ...
  const areReturnSeatsAvailable = useMemo(() => {
    ...
    return selectedReturnRoute.bus.layout.seats.some(
  ...
  const handleReturnDateSelect = async (date: Date) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/routes/search?originId=${selectedRoute.destinationId}&destinationId=${selectedRoute.originId}&date=${routeDate}`,
      );
      ...
      toast({
        title: "Search Error",
        description: "Could not fetch return trips. Please try again.",
        variant: "destructive",
      });
    } finally {
  ...
  const { subtotal, discountAmount, finalPrice, appliedDiscounts } =
    useMemo(() => {
      const outboundSeats = selectedSeats.length;
      const returnSeats = selectedReturnSeats.length;

      const outboundPrice = Number(selectedRoute.price) * outboundSeats;
      const returnPrice = selectedReturnRoute
        ? Number(selectedReturnRoute.price) * returnSeats
        : 0;

      let subtotal = outboundPrice + returnPrice;

      let totalDiscountAmount = 0;
      let appliedDiscountParts: {
        name: string;
        value: string;
        amount: number;
      }[] = [];
      ...
      if (isDiscountActive && ticketCountForDiscount > 0) {
        ...
        if (discount.type === "DATE_BASED" && discount.percentage) {
          ...
          if (applicableTier) {
            ...
            appliedDiscountParts.push({
              name: discount.name,
              value: `${Number(applicableTier.percentage)}%`,
              amount: generalDiscountAmount,
            });
          }
        }
      ...
      if (
        isRoundTrip &&
        selectedReturnRoute &&
        selectedRoute.roundTripDiscountValue
      ) {
        ...
        if (selectedRoute.roundTripDiscountType === "PERCENTAGE") {
          ...
          appliedDiscountParts.push({
            name: "Round Trip Offer",
            value: `${roundTripDiscountValue}%`,
            amount: roundTripDiscountAmount,
          });
        } else if (selectedRoute.roundTripDiscountType === "FIXED") {
          roundTripDiscountAmount = roundTripDiscountValue;
          appliedDiscountParts.push({
            name: "Round Trip Offer",
            value: `${roundTripDiscountValue} ETB`,
            amount: roundTripDiscountAmount,
          });
        }
      ...
      return {
        ...
        finalPrice,
        appliedDiscounts: appliedDiscountParts,
      };
    }, [
      selectedSeats,
      selectedReturnSeats,
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectedSeats.length === 0) {
      toast({
        ...
        description: "Please select seats for the outbound trip.",
        variant: "destructive",
      });
      return;
    }
    if (isRoundTrip && !selectedReturnRoute) {
      toast({
        title: "Return trip required",
        description: "Please select a return trip.",
        variant: "destructive",
      });
      return;
    }
    if (isRoundTrip && selectedReturnSeats.length === 0) {
      toast({
        title: "No return seats selected",
        description: "Please select seats for the return trip.",
        variant: "destructive",
      });
      return;
    }
    if (isRoundTrip && selectedSeats.length !== selectedReturnSeats.length) {
      toast({
        ...
        description:
          "The number of seats for outbound and return trips must be the same.",
        variant: "destructive",
      });
      return;
    }
    ...
    const processBooking = async () => {
      ...
      if (result.success && result.bookingId) {
        ...
        toast({
          ...
          description:
            "Please complete the payment to finalize your ticket purchase.",
        });

        const paymentResult = await createPaymentRequestAction(
          result.bookingId,
          finalPrice,
          authToken,
        );
        ...
        if (paymentResult.success && paymentResult.paymentToken) {
          if (
            ...
            window.myJsChannel.postMessage({
              token: paymentResult.paymentToken,
            });
            toast({
            ...
            console.error(
              "NIB Super App channel (window.myJsChannel) not found.",
            );
            toast({
  ...
  return (
    <>
      <form onSubmit={handleSubmit}>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Review your trip details and select your seats.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-8">
            <div className="p-4 border rounded-lg bg-muted/30">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-2 text-sm mt-4 text-muted-foreground">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {formatRouteDateTime(selectedRoute.departureTime, "h:mm a")}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <BusIcon className="h-4 w-4" />{" "}
                  <span>{selectedRoute.bus.name}</span>
                </div>
              </div>
            </div>
                  ...
                  <div className="p-4 border rounded-lg bg-muted/30 space-y-4">
                    <div className="space-y-2">
                      ...
                      {availableReturnDates.length > 0 ? (
                        ...
                        <Alert variant="destructive">
                          ...
                          <AlertDescription>
                            There are no return trips scheduled for this route
                            yet.
                          </AlertDescription>
                        </Alert>
                      )}
                        ...
                        <RadioGroup
                          onValueChange={(id) =>
                            setSelectedReturnRoute(
                              returnRoutes.find((r) => r.id === id) || null,
                            )
                          }
                          className="space-y-2"
                        >
                          {returnRoutes.map((route) => (
                            <Label
                              key={route.id}
                              htmlFor={route.id}
                              className="flex items-start gap-4 p-3 border rounded-md cursor-pointer hover:bg-muted has-[input:checked]:bg-primary has-[input:checked]:text-primary-foreground has-[input:checked]:border-primary"
                            >
                              <RadioGroupItem
                                value={route.id}
                                id={route.id}
                              ...
                              <div className="flex-grow grid grid-cols-1 sm:grid-cols-3 gap-x-4 gap-y-1 text-sm">
                                ...
                                <div className="font-bold text-base text-right col-span-full sm:col-start-3 sm:row-start-1">
                                  {Number(route.price).toFixed(2)} ETB
                                </div>
                              </div>
                            </Label>
                          ))}
                    ...
                    {!isSearchingReturn && noReturnTripsFound && (
                      <Alert variant="destructive">
                        ...
                        <AlertDescription>
                          There are no available trips for the selected return
                          date.
                        </AlertDescription>
                      </Alert>
                    )}
            ...
            <div>
              <h3 className="font-semibold text-lg flex items-center gap-2 mb-2">
                <Armchair /> Select Your Seats (Outbound)
              </h3>
              {areSeatsAvailable ? (
                <SeatMap
                ...
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    Unfortunately, this bus is fully booked. Please select
                    another bus or time.
                  </AlertDescription>
                </Alert>
              )}
            ...
            {isRoundTrip && selectedReturnRoute && (
              <div>
                <h3 className="font-semibold text-lg flex items-center gap-2 mb-2">
                  <Armchair /> Select Your Seats (Return)
                </h3>
                {areReturnSeatsAvailable ? (
                  ...
                  <Alert variant="destructive">
                    ...
                    <AlertTitle>No Seats Available</AlertTitle>
                    <AlertDescription>
                      This return bus is fully booked. Please select another
                      time or date.
                    </AlertDescription>
                  </Alert>
                )}
            ...
            <div>
              <h3 className="font-semibold text-lg flex items-center gap-2 mb-4">
                <span className="font-bold">ETB</span> Price Summary
              </h3>
              <div className="space-y-2 p-4 border rounded-lg bg-muted/30">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span>
                    {selectedSeats.length} x {selectedRoute.price.toFixed(2)}{" "}
                    ETB
                  </span>
                </div>
                {isRoundTrip && selectedReturnRoute && (
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground flex items-center gap-2">
                      <ArrowRight className="transform -scale-x-100" />
                      Return Tickets
                    </span>
                    <span>
                      {selectedReturnSeats.length} x{" "}
                      {selectedReturnRoute.price.toFixed(2)} ETB
                    </span>
                  </div>
                )}
                <div className="flex justify-between items-center text-sm">
                ...
                {appliedDiscounts.map((discount, index) => (
                  <div
                    key={index}
                    className="flex justify-between items-center text-sm text-green-600 font-semibold"
                  >
            ...
            {isRoundTrip &&
              ...
              selectedSeats.length !== selectedReturnSeats.length && (
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    Please select the same number of seats for the outbound and
                    return trips. You have selected {selectedSeats.length}{" "}
                    outbound and {selectedReturnSeats.length} return seats.
                  </AlertDescription>
                </Alert>
              )}
      ...
      <AlertDialog open={showConfirmation} onOpenChange={setShowConfirmation}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Purchase Confirmed!</AlertDialogTitle>
            <AlertDialogDescription>
              Your seat selection is saved. You can view your ticket status now,
              and the QR code will appear once payment is confirmed.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <Button
      ...
      <AlertDialog
        open={showWebPaymentAlert}
        onOpenChange={setShowWebPaymentAlert}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Thank you for your interest! Currently, our payment system is
              exclusively available through the{" "}
              <strong>NibTera Super App</strong>.<br />
              Web-based payments are coming soon. Please complete your ticket
              purchase and payment using the <strong>Mini App</strong> section
              of the <strong>NibTera Super App</strong> for now.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
