...
function Seat({ seat, onSelect, isSelectable }: SeatProps) {
  const finalIsSelectable =
    isSelectable && seat.type === "SEAT" && seat.status !== "OCCUPIED";
  ...
  const seatClasses = cn(
    ...
    seat.status === "AVAILABLE" &&
      "bg-green-100 border-green-400 text-green-800 hover:bg-green-200 hover:border-green-600 cursor-pointer dark:bg-green-900/50 dark:border-green-800 dark:text-green-300 dark:hover:bg-green-900",
    seat.status === "OCCUPIED" &&
      "bg-muted border-muted-foreground/30 text-muted-foreground cursor-not-allowed opacity-70",
    seat.status === "SELECTED" &&
      "bg-accent border-accent-foreground text-accent-foreground cursor-pointer shadow-lg scale-110",
  ...
  return (
    <div
      ...
      title={
        seat.type === "SEAT" ? `Seat ${seat.seatNumber} - ${seat.status}` : ""
      }
    >
...
export function SeatMap({
  ...
  const getInitialSeats = () => {
    const occupiedSeatNumbers = new Set(tickets.map((t) => t.seatNumber));
    ...
    setSeats((currentSeats) => {
      ...
      if (seat?.status === "SELECTED") {
        ...
        return currentSeats.map((s) =>
          s.seatNumber === seatNumber ? { ...s, status: "AVAILABLE" } : s,
        );
      }

      const selectedSeatsCount = currentSeats.filter(
        (s) => s.status === "SELECTED",
      ).length;
      if (selectedSeatsCount >= 10) {
        toast({
          title: "Selection Limit",
          description: "You can select a maximum of 10 seats per purchase.",
          variant: "destructive",
        });
        return currentSeats;
      }
      ...
      // Select the seat
      return currentSeats.map((s) =>
        s.seatNumber === seatNumber ? { ...s, status: "SELECTED" } : s,
      );
    });
  };
    ...
    <div className="flex flex-col items-center pt-4">
      <ScrollArea className="w-full">
        <div
          ...
          style={{
            gridTemplateColumns: `repeat(${bus.layout.cols}, minmax(0, 1fr))`,
          }}
        >
          {seats.map((seat) => (
            <Seat
      ...
      <div className="flex justify-center flex-wrap gap-x-4 gap-y-2 mt-4 text-sm">
        <div className="flex items-center">
          <Armchair className="w-4 h-4 mr-2 text-green-600 dark:text-green-400" />
          Available
        </div>
        <div className="flex items-center">
          <Armchair className="w-4 h-4 mr-2 text-accent" />
          Selected
        </div>
        <div className="flex items-center">
          <Armchair className="w-4 h-4 mr-2 text-muted-foreground" />
          Occupied
        </div>
      </div>
    </div>
