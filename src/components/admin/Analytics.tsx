...
export function Analytics({ bookings, routes, buses }: AnalyticsProps) {
  ...
  const todayStats = useMemo(() => {
    ...
    const soldRouteIds = new Set(
      todayBookings.map((booking) => booking.routeId),
    );
    ...
    const actualRevenue = todayBookings.reduce(
      (acc, booking) => acc + Number(booking.totalPrice),
      0,
    );

    const effectiveness =
      potentialRevenue > 0 ? (actualRevenue / potentialRevenue) * 100 : 0;
    ...
    <div className="space-y-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Performance metrics for ticket purchases made today.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="p-4 border rounded-lg">
              ...
              <p className="text-3xl font-bold text-primary">
                {todayStats.actualRevenue.toLocaleString("en-US", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </p>
              <p className="text-xs text-muted-foreground">
                Revenue from tickets purchased today.
              </p>
            </div>
            <div className="p-4 border rounded-lg">
              <h4 className="text-sm font-semibold text-muted-foreground flex items-center mb-2">
              ...
              <p className="text-3xl font-bold">
                {todayStats.potentialRevenue.toLocaleString("en-US", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </p>
              <p className="text-xs text-muted-foreground">
                If all seats on routes sold today were purchased.
              </p>
            </div>
            <div className="p-4 border rounded-lg">
              <h4 className="text-sm font-semibold text-muted-foreground flex items-center mb-2">
              ...
              <div className="flex items-center gap-4">
                <Progress
                  value={todayStats.effectiveness}
                  className="w-full h-3"
                />
                <span className="text-2xl font-bold">
                  {todayStats.effectiveness.toFixed(1)}%
                </span>
              </div>
              <p className="text-xs text-muted-foreground mt-2">
                Today's sales vs. potential revenue.
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Performance breakdown of your most successful routes based on
                revenue.
              </CardDescription>
            </div>
            <Popover>
              ...
              <div className="md:hidden space-y-4">
                  ...
                  <div key={stat.routeId} className="p-4 border rounded-lg">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm">
                      ...
                      <div className="flex items-center gap-1 text-muted-foreground">
                        ...
                        {stat.potentialRevenue.toFixed(2)} ETB
                      </div>
                      <div className="flex items-center gap-1">
                ...
                <div className="p-4 border rounded-lg bg-muted/50 font-bold space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span>{totalRevenue.toFixed(2)} ETB</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Total Potential:</span>
                    <span>{totalPotentialRevenue.toFixed(2)} ETB</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Total Effectiveness:</span>
                      ...
                      {routeStats.map((stat) => (
                        <TableRow key={stat.routeId}>
                          ...
                          <TableCell className="text-right text-muted-foreground">
                            {stat.potentialRevenue.toFixed(2)}
                          </TableCell>
                          <TableCell className="text-right">
                            <Badge className="bg-green-600 hover:bg-green-700">
                              {totalRevenue > 0
                                ? ((stat.revenue / totalRevenue) * 100).toFixed(
                                    1,
                                  )
                                : "0.0"}
                              %
