...
import type { Bus, SeatLayout } from "@prisma/client";

interface EditBusFormProps {
    bus: Bus & { layout: SeatLayout | null }
}
...
export function EditBusForm({ bus }: EditBusFormProps) {
    ...
    // Note: We are not trying to reverse-engineer the aisle columns from the seat layout.
    // The user must re-define the layout. This is simpler and avoids complex logic.
    ...
    const [aisleCols, setAisleCols] = useState('3'); // Default to a common layout
    ...
    return (
        <form ref={formRef} onSubmit={handleSubmit}>
            ...
            <Card className="max-w-xl mx-auto">
                <CardHeader>
                    <CardTitle>Edit Bus: {bus.name}</CardTitle>
                    <CardDescription>
                        Update the properties and seat layout for this bus.
                        <span className="font-bold text-destructive block mt-2">Warning: Changing the layout will permanently delete the old one. This cannot be undone.</span>
                    </CardDescription>
                </CardHeader>
                <CardContent>
