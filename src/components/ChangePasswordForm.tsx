...
export function ChangePasswordForm({ csrfToken }: ChangePasswordFormProps) {
  ...
  const [clientCsrf, setClientCsrf] = useState<string | undefined>(csrfToken);
    ...
    if (!csrfToken) {
      ...
      (async () => {
        try {
          ...
          if (mounted && data?.token) setClientCsrf(data.token);
        } catch (e) {
          // ignore fetch errors; form will show invalid session if missing
        }
      })();
      return () => {
        mounted = false;
  ...
  const handleSubmit = (formData: FormData) => {
    ...
    startTransition(async () => {
      if (!clientCsrf) {
        toast({
          title: "Error",
          description: "Invalid session. Please refresh.",
          variant: "destructive",
        });
        return;
      }
  ...
  return (
    <Card className="max-w-xl border-0 shadow-none">
      <form ref={formRef} action={handleSubmit}>
        <input type="hidden" name="csrfToken" value={clientCsrf || ""} />
